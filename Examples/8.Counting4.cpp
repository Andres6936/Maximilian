...
void play(double* output)
{
	...
	CurrentCount = myCounter.phasor(1 * ((another.sawn(0.1) + 1) / 2), 1,
			9);//phasor can take three arguments; frequency, start value and end value.
	...
	{//and the 'else' bit.
		...
		myOscOutput = mySwitchableOsc.sawn(
				myArray[CurrentCount]);//one osc object can produce whichever waveform you want.
	}
